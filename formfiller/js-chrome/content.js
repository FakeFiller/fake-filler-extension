var FormFiller = function ($, options, analyticsTrackingCode) {
    var
    ...
    return {
        ...
        trackEvent: function(category, action) {
            if (!analyticsTracker) {
                analyticsTracker = analyticsService.getTracker(analyticsTrackingCode);
            }
            analyticsTracker.sendEvent(category, action);
        }
    };
};
...
var analyticsService = analytics.getService('Form Filler Extension'),
    analyticsTracker;
...
chrome.runtime.sendMessage('getOptions', function (response) {
    if (!window.formFiller) {
        window.formFiller = new FormFiller(jQuery, response.options, response.analyticsTrackingCode);
    }
});
